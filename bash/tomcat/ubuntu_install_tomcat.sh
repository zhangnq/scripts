...
echo "============================jdk==============================="
read -p "Please input the java version(1.7,1.8):" java_version
if [ "$java_version" = "" ];then
        echo "You must input java version."
        exit 1
fi
...
#sed -i '/# OS specific support.*/a\JAVA_HOME="$java_home"\nJAVA_OPTS="-server -Xms256M -Xmx1024M -XX:MaxPermSize=512M -Dfile.encoding=utf-8"' /usr/local/$project/bin/catalina.sh
sed -i "/# OS specific support.*/a\\JAVA_HOME=\"$java_home\"\\nJAVA_OPTS=\"-server -Xms256M -Xmx4096M -XX:MaxPermSize=1024M -Dfile.encoding=utf-8\"" /usr/local/$project/bin/catalina.sh
...
sed -i 's@<Connector port="8080" protocol="HTTP/1.1"@<!-- <Connector port="8080" protocol="HTTP/1.1"@g' /usr/local/$project/conf/server.xml
sed -i 's@<!-- A "Connector" using the shared thread pool-->@--><!-- A "Connector" using the shared thread pool-->@g' /usr/local/$project/conf/server.xml

sed -i "s@<Connector port=\"8009\" protocol=\"AJP/1.3\"@<Connector port=\"$ajp_port\" address=\"127.0.0.1\" protocol=\"AJP/1.3\"@g" /usr/local/$project/conf/server.xml
...
<tomcat-users>
 <role rolename="tomcat"/>
  ...
  <user username="tomcat" password="PasSw0rld@#" roles="tomcat,manager-gui,manager-script,manager-status,manager-jmx"/>
</tomcat-users>
EOF

rm -rf /usr/local/$project/webapps/*

wget http://nb.cyyun.com:18104/scripts/bash/service/tomcat7 -O /etc/init.d/$project
...
wget http://nb.cyyun.com:18104/scripts/bash/tomcat/cut_tomcat_logs -O /etc/cron.daily/cut_tomcat_logs
